...
# added pentation and negative infinity at version 1.0.3
from math import *
class BigNumber:

    def __init__(self, mantissa=0, exponent=0, layer=0, pentate=0):
    ...
    def add(self, other):
        ...
        return BigNumber(m, e, self.l, self.pe)
    def sub(self, other):
        if self.pe > other.pe:
            return self
    ...
    # IMPORTANT: limit is 10 ^^^ 10 ^ 308 (pentation is "^^^")
    def tostring(self):
        if self.pe > 0:

            if self.pe < 10:
                return "^" * (self.pe + 2) + str(round(self.l, 3))
...
BigNumber.NEG_INF = BigNumber(-1, 1, 308)
